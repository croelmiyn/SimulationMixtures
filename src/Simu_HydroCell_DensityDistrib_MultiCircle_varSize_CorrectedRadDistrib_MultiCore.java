...
public class Simu_HydroCell_DensityDistrib_MultiCircle_varSize_CorrectedRadDistrib_MultiCore implements PlugIn{
    ...
    the flow field is the average flow field, which is constant, because the configuration of swimmers is constant, throughout the simulation

    2023 05 18: corrected the distribution of radii of curvature to double gaussian
     */
    ...
    public void run(String arg) {
        ...
        gd.addNumericField("Fraction_ShortRadii",0.4,1);
        ...
        final double m1 = (elongated)?1.75:1.155; // mean( -log10(Curvature) ) shorter circles
        final double m2 = (elongated)?1.90:1.554; // mean( -log10(C) ) larger circles
        final double s1 = (elongated)?0.35:0.2187; // std( -log10(C) ) shorter circles
        final double s2 = (elongated)?0.20:0.3451; // std( -log10(C) ) larger circles
        final double f = gd.getNextNumber(); // fraction of small radii (m1) in the double Gaussian ( log normal actually ) distribution of radii
        // standard should be to use f= 0.5 if elongated, and f = 0.4 for normal size
            ...
            futures[l] = ConcurrencyUtils.submit( () ->
            {
                ...
                for(int k=firstK; k<lastK; k++){
                    ...
                    synchronized (rd) { // synchronized for unique random number generator
                        ...
                        // e<f: m1,s1  e>f: m2,s2
                        double logC = (e<f)? m1+s1*rd.nextGaussian() : m2+s2*rd.nextGaussian()  ; // double Gaussian
                        rad = Math.pow(10,logC)/nx; // 1 unit = nx mum standard, in px
                        if(rad<3.0/nx){rad=3.0/nx;} // we forbid radius of gyration smaller than 3 µm
                        rad/=nx;
                    }
